...
function Field({ label, children, hint }) {
  return (
    <label className="block">
      ...
      {hint ? (
        <span className="mt-1 block text-xs text-sky-100/62">{hint}</span>
      ) : null}
    </label>
  );
}
...
export default function CustomerAccountOnboardingPage() {
  ...
  const accountType = isPropertyManager
  ...
  const submitAccount = async (event) => {
    ...
    try {
      ...
      const accountExists =
        ...
        /account.*email.*already exists|email.*already exists/i.test(
          String(message || '')
        );
      if (accountExists) {
        setLoginForm({
  ...
  const submitLogin = async (event) => {
    ...
    try {
      ...
      const access =
        loginResponse.data?.access || loginResponse.data?.access_token;
      const refresh =
        loginResponse.data?.refresh || loginResponse.data?.refresh_token;
      if (!access)
      ...
      const profiles = Array.isArray(data?.property_profiles)
        ? data.property_profiles
        : [];
      const setupProfile = profiles.find((profile) =>
        propertyNeedsSetup(profile)
      );
      const hasReadyProperty = profiles.some(
        (profile) => !propertyNeedsSetup(profile)
      );
      if (hasReadyProperty && token) {
        navigate(
          customerContinuationDestination(requestedNext, token) ||
  ...
  const openExistingPortal = () => {
    ...
    navigate(
      email
        ? `/portal?email=${encodeURIComponent(email)}${roleQuery}`
        : '/portal'
    );
  };
  ...
  const submitProperty = async (event) => {
    ...
    if (!portalToken) {
      setError(
        'We could not load your customer portal yet. Please sign in again.'
      );
      setStep('signin');
      return;
    }
    ...
    try {
      ...
      if (propertyProfileId) {
        await api.patch(
          `/projects/customer-portal/${encodeURIComponent(portalToken)}/property/`,
          payload
        );
      } else {
        await api.post(
    ...
    <div className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_35%_8%,rgba(37,99,235,0.26),transparent_28%),linear-gradient(135deg,#020617_0%,#082044_52%,#0f172a_100%)] text-white">
      <header className="border-b border-white/10 bg-slate-950/80">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <Link
            to="/portal"
            className="rounded-xl border border-white/16 px-4 py-2 text-sm font-semibold text-sky-50 hover:bg-white/8"
          >
            Customer Log In
          </Link>
        </div>
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              [
                'No project required',
                'Create your account now and start a project later.',
              ],
              [
                'Secure portal',
                'Verify your email before your property dashboard opens.',
              ],
              [
                'No duplicates',
                'Future project requests link back to this account by email.',
              ],
              [
                'Property-first',
                'Add the address once so future requests are easier.',
              ],
            ].map(([title, text]) => (
              <div
                key={title}
              ...
              >
                ...
                <div className="mt-1 text-sm leading-6 text-sky-100/66">
                  {text}
                </div>
              </div>
            ))}
        ...
        <section className="rounded-3xl border border-white/12 bg-slate-950/58 p-5 shadow-2xl shadow-slate-950/35 sm:p-6">
          ...
          {error ? (
            <div
              ...
              className="mb-4 rounded-2xl border border-rose-300/30 bg-rose-500/12 p-3 text-sm text-rose-100"
            >
              {error}
            </div>
          ) : null}
          ...
          {step === 'account' ? (
            <form
              data-testid="customer-account-create-form"
              className="space-y-4"
            ...
            >
              ...
              >
                <TextInput
                  data-testid="customer-account-email"
                  type="email"
              ...
              <Field label="Phone">
                <TextInput
                  data-testid="customer-account-phone"
              ...
              <Field label="Password">
                <TextInput
                  data-testid="customer-account-password"
                  type="password"
              ...
              <Field label="Confirm password">
                <TextInput
                  data-testid="customer-account-password-confirm"
                  type="password"
                  value={accountForm.password_confirm}
                  onChange={(e) =>
                    updateAccount('password_confirm', e.target.value)
                  }
                  required
                  autoComplete="new-password"
                />
          ...
          {step === 'verify' ? (
            <div
              data-testid="customer-account-verify-step"
              className="space-y-5"
            >
              <div className="rounded-2xl border border-emerald-300/28 bg-emerald-400/10 p-4">
                ...
                <p className="mt-3 text-sm leading-6 text-sky-50/76">
                  ...
                  {accountForm.email || loginForm.email}. Verify your email,
                  then sign in here to add your first property.
                </p>
              </div>
              <button
                type="button"
          ...
          {step === 'existing' ? (
            <div
              data-testid="customer-account-existing-step"
              className="space-y-5"
            >
              <div className="rounded-2xl border border-sky-300/28 bg-sky-400/10 p-4 text-sm leading-6 text-sky-50/82">
                <div className="font-semibold text-white">
                  This email already has a MyHomeBro login.
                </div>
                <p className="mt-2">
                  Open the Customer Portal and use your existing email and
                  password. Your customer and contractor workspaces can share
                  the same login.
                </p>
              </div>
              <button
          ...
          {step === 'signin' ? (
            <form
              data-testid="customer-account-signin-form"
              className="space-y-4"
              onSubmit={submitLogin}
            >
              <div className="rounded-2xl border border-sky-300/28 bg-sky-400/10 p-4 text-sm leading-6 text-sky-50/80">
                Your email is verified. Sign in with the password you just
                created to finish {accountLabel.toLowerCase()} setup.
              </div>
              <Field label="Email">
                <TextInput
                  data-testid="customer-account-signin-email"
                  type="email"
              ...
              <Field label="Password">
                <TextInput
                  data-testid="customer-account-signin-password"
                  type="password"
              ...
              <button
                data-testid="customer-account-signin-submit"
                type="submit"
                disabled={loading}
                className="w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-3 font-semibold text-white shadow-lg shadow-blue-950/25 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-60"
              >
          ...
          {step === 'property' ? (
            <form
              data-testid="customer-account-property-form"
              className="space-y-4"
            ...
            >
              <div className="rounded-2xl border border-blue-300/24 bg-blue-500/10 p-4 text-sm leading-6 text-sky-50/76">
                ...
                <p className="mt-2">
                  This gives your dashboard a home base. You can start a project
                  later.
                </p>
              </div>
              <Field label="Property nickname">
                <TextInput
                  data-testid="customer-account-property-name"
                  value={propertyForm.display_name}
                  onChange={(e) =>
                    updateProperty('display_name', e.target.value)
                  }
                  required
                />
              </Field>
              <Field label="Property type">
                <SelectInput
                  data-testid="customer-account-property-type"
                  value={propertyForm.property_type}
                  onChange={(e) =>
                    updateProperty('property_type', e.target.value)
                  }
                >
              ...
              <Field label="Street address">
                <TextInput
                  data-testid="customer-account-property-address"
                  value={propertyForm.address_line1}
                  onChange={(e) =>
                    updateProperty('address_line1', e.target.value)
                  }
                  required
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <Field label="City">
                  <TextInput
                    data-testid="customer-account-property-city"
                ...
                <Field label="State">
                  <TextInput
                    data-testid="customer-account-property-state"
                ...
                <Field label="ZIP">
                  <TextInput
                    data-testid="customer-account-property-zip"
                    value={propertyForm.postal_code}
                    onChange={(e) =>
                      updateProperty('postal_code', e.target.value)
                    }
                    required
                  />
                </Field>
              ...
              <button
                data-testid="customer-account-property-submit"
                type="submit"
                disabled={loading}
                className="w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-3 font-semibold text-white shadow-lg shadow-blue-950/25 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-60"
              >
          ...
          {step === 'dashboard' ? (
            <div
              data-testid="customer-account-dashboard-ready"
              className="space-y-5"
            >
              <div className="rounded-2xl border border-emerald-300/28 bg-emerald-400/10 p-5">
                ...
                <p className="mt-3 text-sm leading-6 text-sky-50/76">
                  Your property is saved. When you submit a project later,
                  MyHomeBro will link it to this account automatically.
                </p>
              </div>
              <button
                type="button"
                data-testid="customer-account-go-dashboard"
                onClick={() =>
                  navigate(
                    customerContinuationDestination(
                      requestedNext,
                      portalToken
                    ) || `/portal/${encodeURIComponent(portalToken)}`
                  )
                }
                className="w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-3 font-semibold text-white shadow-lg shadow-blue-950/25 hover:from-blue-500 hover:to-indigo-500"
              >
