...
function outreachStatusLabel(row) {
  const status = safeText(row?.outreach_status);
  if (status) return titleize(status);
  if (row?.claimed) return "Claimed";
  if (row?.contact_status === "manual_review_needed") return "Manual Review Needed";
  return "No Outreach";
}

function outreachStatusClass(row) {
  const status = safeText(row?.outreach_status || (row?.claimed ? "claimed" : ""));
  if (status === "claimed") return "border-emerald-200/35 bg-emerald-300/15 text-emerald-50";
  if (status === "manual_review_needed") return "border-amber-200/40 bg-amber-300/15 text-amber-50";
  if (["phone_outreach_logged", "website_outreach_logged", "claim_link_generated", "email_outreach_logged", "sms_outreach_logged", "outreach_logged"].includes(status)) {
    return "border-sky-200/35 bg-sky-300/15 text-sky-50";
  }
  return "border-white/15 bg-white/8 text-sky-100/70";
}

function outreachSuccessMessage(outreachType) {
  if (outreachType === "phone") return "Phone outreach logged";
  if (outreachType === "website_form") return "Website form outreach logged";
  if (outreachType === "manual_note") return "Manual review status updated";
  return "Outreach activity logged";
}

function optimisticOutreachRow(row, outreachType, notes) {
  const outreachStatus = {
    phone: "phone_outreach_logged",
    website_form: "website_outreach_logged",
    claim_link_copied: "claim_link_generated",
    manual_note: "manual_review_needed",
  }[outreachType] || "outreach_logged";
  return {
    ...row,
    contact_status: outreachType === "manual_note" ? "manual_review_needed" : row.contact_status,
    outreach_notes: outreachType === "manual_note" ? notes : row.outreach_notes,
    outreach_status: outreachStatus,
    outreach_attempt_count: Number(row.outreach_attempt_count || 0) + 1,
    latest_outreach_type: outreachType,
    latest_outreach_status: outreachType === "claim_link_copied" ? "generated" : "logged",
    latest_outreach_at: new Date().toISOString(),
    latest_outreach_notes: notes,
  };
}
...
export default function AdminContractorDirectory() {
  ...
  function updateDirectoryRow(updatedRow) {
    if (!updatedRow?.id) return;
    setDirectoryRows((prev) => prev.map((row) => (row.id === updatedRow.id ? { ...row, ...updatedRow } : row)));
  }
  ...
  async function generateClaimLink(row) {
    ...
    try {
      ...
      updateDirectoryRow(data?.entry || optimisticOutreachRow(row, "claim_link_copied", "Claim link generated by admin."));
  ...
  async function logOutreach(row, outreachType, notes) {
    ...
    setSuccessMessage("");
    try {
      const { data } = await api.post(`/projects/admin/contractor-directory/${row.id}/outreach-log/`, {
        outreach_type: outreachType,
        destination: outreachType === "phone" ? row.phone : outreachType === "website_form" ? row.contact_form_url : "",
        notes,
      });
      updateDirectoryRow(data?.entry || optimisticOutreachRow(row, outreachType, notes));
      setSuccessMessage(outreachSuccessMessage(outreachType));
    } catch (error) {
