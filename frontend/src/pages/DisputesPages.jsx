...
function EscrowAllocationPanel({ dispute, isAdmin, onChanged }) {
  ...
  return <WorkspaceSection title="Escrow Allocation" eyebrow="Explicit financial instructions" tone="warning">
    ...
    {allocations.map((row) => <div key={row.id} className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950"><div className="flex flex-wrap justify-between gap-2"><strong>{money(row.contractor_amount_cents / 100)} contractor / {money(row.homeowner_amount_cents / 100)} homeowner</strong><Badge tone={["ready_for_execution", "executed"].includes(row.status) ? "good" : "warn"}>{row.status === "ready_for_execution" && row.execution_supported === false ? "validated — manual processing" : String(row.status).replaceAll("_", " ")}</Badge></div><div className="mt-1">{row.explanation}</div>{row.execution_error ? <div className="mt-2 rounded-lg border border-rose-300 bg-rose-50 p-2 text-rose-900">Retryable execution issue: {row.execution_error}</div> : null}{isAdmin && row.status === "ready_for_execution" && row.execution_supported === false ? <div className="mt-2 rounded-lg border border-amber-300 bg-amber-100 p-2 font-semibold text-amber-950">{row.execution_blocker || "This allocation requires manual payment processing."}</div> : null}{isAdmin && row.status === "ready_for_execution" && row.execution_supported !== false && row.execution_enabled !== true ? <div className="mt-2 rounded-lg border border-sky-200 bg-sky-50 p-2 font-semibold text-sky-950">Authorization and staff validation are complete. Payment execution is disabled until payment operations enables it.</div> : null}<div className="mt-2 flex flex-wrap gap-2">{!row.contractor_authorized_at && !isAdmin ? <button className="mhb-btn" type="button" onClick={() => authorize(row.id)}>Authorize Contractor Allocation</button> : null}{isAdmin && row.status === "authorized" ? <button className="mhb-btn primary" type="button" onClick={() => confirm(row.id)}>Validate for Execution</button> : null}{isAdmin && row.status === "ready_for_execution" && row.execution_supported !== false && row.execution_enabled === true ? <button className="mhb-btn primary" type="button" onClick={() => execute(row.id)}>Execute Authorized Allocation</button> : null}</div></div>)}
    ...
    <div className="grid gap-4" data-testid="resolution-case-page">
      ...
      <section id="resolution-response" className="scroll-mt-52 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          {!isClosed(dispute) ? <button className="mhb-btn primary" onClick={() => onOpenRespond()} disabled={!canRespond(dispute)} type="button">Send Customer Message</button> : null}
        </div>
...
function RespondModal({ open, dispute, initialText = "", onClose, onSubmitted }) {
  ...
  return (
    <ModalShell title={`Send Customer Message — Case #${dispute.id}`} onClose={onClose}>
