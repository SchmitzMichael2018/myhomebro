...
export default function PublicEstimateReviewPage() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-slate-950 px-3 py-6 text-slate-100 sm:px-6" data-testid="customer-estimate-review">
    <div className="mx-auto max-w-4xl space-y-4">
      ...
      <section className="rounded-2xl border border-white/10 bg-slate-900 p-5"><h2 className="text-xl font-black">Pricing</h2><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[34rem] text-left"><thead className="text-sm text-slate-400"><tr><th className="py-2">Description</th><th>Quantity</th><th className="text-right">Total</th></tr></thead><tbody>{(pricing.line_items || []).map((item, i) => <tr key={`${item.description}-${i}`} className="border-t border-white/10"><td className="py-3"><strong>{item.description}</strong><span className="block text-xs text-slate-400">{item.category_label}</span></td><td>{item.quantity} {item.unit}</td><td className="text-right">{money(item.total)}</td></tr>)}</tbody></table></div><dl className="ml-auto mt-4 max-w-xs space-y-2"><div className="flex justify-between"><dt>Subtotal</dt><dd>{money(pricing.subtotal)}</dd></div>{Number(pricing.tax) ? <div className="flex justify-between"><dt>Tax</dt><dd>{money(pricing.tax)}</dd></div> : null}{Number(pricing.discounts) ? <div className="flex justify-between"><dt>Discounts</dt><dd>-{money(pricing.discounts)}</dd></div> : null}<div className="flex justify-between border-t border-white/15 pt-3 text-xl font-black"><dt>Total</dt><dd>{money(pricing.total)}</dd></div></dl></section>
