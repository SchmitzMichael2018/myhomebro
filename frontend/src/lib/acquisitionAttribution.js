const ALLOWED_EVENTS = new Set([
  'landing_view', 'improvement_template_view', 'guide_view', 'signup_started',
  'role_selected', 'diy_project_started', 'hire_pro_clicked',
  'contractor_profile_viewed', 'contractor_invite_started',
]);

const clean = (value, maximum = 100) => String(value || '')
  .trim()
  .replace(/[^a-zA-Z0-9 _./:+-]/g, '')
  .slice(0, maximum);

export function attributionPayload(eventType, location, extra = {}) {
  if (!ALLOWED_EVENTS.has(eventType)) throw new Error('Unsupported public attribution event.');
...
export async function trackAcquisitionEvent(eventType, location = window.location, extra = {}) {
...
export function publicRouteEvent(pathname) {
  ...
  if (pathname.includes('improvement')) return 'improvement_template_view';
  if (['/register', '/signup', '/create-account'].includes(pathname)) return 'signup_started';
