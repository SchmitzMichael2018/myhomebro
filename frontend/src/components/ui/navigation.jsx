    ...
    <header className={cx("space-y-4", theme === "operational" && "border-b border-[var(--mhb-border-divider)] pb-5", className)} {...props}>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          {eyebrow ? <div className={cx("mb-1 text-[11px] font-semibold uppercase tracking-[0.2em]", theme === "operational" ? "text-[var(--mhb-text-muted)]" : "text-slate-500")}>{eyebrow}</div> : null}
        ...
        <div className="flex flex-wrap gap-2">
          {secondaryActions}
          {onOpenProjectAssistant ? <AIActionButton theme={theme} onClick={onOpenProjectAssistant}>Project Assistant</AIActionButton> : null}
          {primaryAction}
        </div>
      </div>
    </header>
...
export function FilterToolbar({ label = "Table filters", filters = null, search = null, actions = null, activeFilters = null, theme = "default", className = "", ...props }) {
  return (
    <section aria-label={label} className={cx(
      ...
      {activeFilters ? <div className={cx("mt-3 flex flex-wrap gap-2 border-t pt-3", theme === "operational" ? "border-[var(--mhb-border-divider)]" : "border-slate-100")}>{activeFilters}</div> : null}
    </section>
  );
}

export function WorkspaceStepNavigation({ steps = [], activeStep, onStepChange, label = "Workspace progress", theme = "default", className = "", ...props }) {
  return (
            ...
            <li key={key}>
              ...
              >
                <span className={cx(
                  ...
                  theme === "operational"
                    ? complete
                      ? "bg-[var(--mhb-status-complete-bg)] text-[var(--mhb-status-complete-text)]"
                      : active
                        ? "bg-[var(--mhb-interactive-primary)] text-[var(--mhb-text-inverse)]"
                        : "bg-[var(--mhb-surface-inset)] text-[var(--mhb-text-muted)]"
                    : complete ? "bg-green-100 text-green-700" : active ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600"
                )}>
...
export function ActionMenu({ label = "More actions", items = [], open, onOpenChange, theme = "default", className = "", ...props }) {
  ...
  return (
    <div className={cx("relative inline-flex", className)} {...props}>
      <Button theme={theme} variant="icon" aria-label={label} aria-haspopup="menu" aria-expanded={isOpen} onClick={() => setOpen(!isOpen)}>
        <MoreHorizontal className="h-5 w-5" aria-hidden="true" />
      </Button>
      {isOpen ? (
        <div role="menu" className={cx(
          "absolute right-0 top-full z-20 mt-2 min-w-48 rounded-xl border p-1.5 shadow-xl",
          theme === "operational"
            ? "border-[var(--mhb-border-default)] bg-[var(--mhb-surface-overlay)] text-[var(--mhb-text-primary)] shadow-[var(--mhb-shadow-card-elevated)]"
            : "border-slate-200 bg-white"
        )}>
          {items.map((item, index) => {
            ...
            return (
              <button
                ...
                className={cx(
                  "flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-semibold disabled:opacity-50",
                  theme === "operational"
                    ? item.danger
                      ? "text-[var(--mhb-status-blocked-text)] hover:bg-[var(--mhb-status-blocked-bg)]"
                      : "text-[var(--mhb-text-secondary)] hover:bg-[var(--mhb-interactive-ghost-hover)] hover:text-[var(--mhb-text-primary)]"
                    : item.danger ? "text-red-700 hover:bg-red-50" : "text-slate-700 hover:bg-slate-100"
                )}
              >
