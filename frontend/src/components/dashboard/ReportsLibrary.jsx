...
function TrendPreview({ title, value, subtitle, data, dataKey, color, onClick }) {
  return (
    <button type="button" aria-label={`Open ${title} report`} onClick={onClick} className="min-w-[230px] rounded-xl border border-slate-200 bg-white p-4 text-left transition hover:-translate-y-0.5 hover:border-blue-400 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
...
export default function ReportsLibrary({
  ...
  const visibleReports = useMemo(
    () => selectedCategory === "all" ? reports : reports.filter((report) => report.category.toLowerCase() === selectedCategory),
    [selectedCategory],
  );
  const categoryOptions = [
    { key: "all", title: "All Reports", description: "Browse every available report.", count: reports.length, icon: FileBarChart2, tone: "bg-blue-50 text-blue-700" },
    ...categories,
  ];
  const shortcutReports = [reports[0], reports[1], reports[3], reports[2]];
    ...
    <div data-testid="reports-library" className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_280px]">
      <div className="space-y-3">
        <section data-testid="reports-categories" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 overflow-x-auto pb-1"><div className="grid min-w-[720px] grid-cols-6 gap-2">
            {categoryOptions.map(({ key, title, description, count, icon: Icon, tone }) => { const active = selectedCategory === key; return <button key={key} type="button" aria-pressed={active} data-testid={`reports-category-${key}`} onClick={() => setSelectedCategory(key)} className={`rounded-xl border p-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${active ? "border-blue-500 bg-blue-50/70 shadow-sm" : "border-slate-200 hover:border-blue-300"}`}><span className={`flex h-9 w-9 items-center justify-center rounded-lg ${active ? "bg-blue-600 text-white" : tone}`}><Icon aria-hidden="true" className="h-5 w-5" /></span><div className={`mt-2 font-bold ${active ? "text-blue-800" : "text-slate-900"}`}>{title}</div><div className="mt-1 min-h-8 text-xs leading-4 text-slate-500">{description}</div><div className={`mt-2 flex items-center justify-between text-xs font-semibold ${active ? "text-blue-700" : "text-slate-600"}`}><span>{count} report{count === 1 ? "" : "s"}</span><ChevronRight aria-hidden="true" className="h-4 w-4" /></div></button> })}
          </div></div>
        </section>

        <section data-testid="reports-popular" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-950">Available Reports</h2><p className="mt-1 text-sm text-slate-500">Reports in the selected category.</p></div><span className="text-sm font-bold text-blue-700">{visibleReports.length} available</span></div>
          {visibleReports.length ? <><div className="mt-4 hidden md:block"><table className="w-full text-left text-sm"><thead className="border-y border-slate-200 bg-slate-50 text-xs text-slate-500"><tr><th className="px-3 py-2">Favorite</th><th className="px-3 py-2">Report</th><th className="px-3 py-2">Category</th><th className="px-3 py-2">Description</th><th className="px-3 py-2">Last Run</th><th className="px-3 py-2">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{visibleReports.map(({ key, title, category, description, icon: Icon, tone }) => <tr key={key} className="hover:bg-slate-50/70"><td className="px-3 py-2"><Heart aria-hidden="true" className="h-4 w-4 text-slate-400" /></td><td className="px-3 py-2"><div className="flex items-center gap-2"><span className={`flex h-8 w-8 items-center justify-center rounded-lg ${tone}`}><Icon aria-hidden="true" className="h-4 w-4" /></span><strong className="text-slate-900">{title}</strong></div></td><td className="px-3 py-2 text-slate-600">{category}</td><td className="px-3 py-2 text-slate-600">{description}</td><td className="px-3 py-2 text-slate-400" aria-label="Never run">—</td><td className="px-3 py-2"><button type="button" data-testid={`run-report-${key}`} onClick={() => run(key)} className="rounded-lg border border-blue-200 px-3 py-1.5 font-bold text-blue-700 hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">Run</button></td></tr>)}</tbody></table></div>
          <div className="mt-3 divide-y divide-slate-100 md:hidden">{visibleReports.map(({ key, title, category, description, icon: Icon, tone }) => <article key={key} className="py-3 first:pt-0"><div className="flex items-start gap-3"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${tone}`}><Icon aria-hidden="true" className="h-4 w-4" /></span><div className="min-w-0 flex-1"><div className="font-bold text-slate-900">{title}</div><div className="mt-0.5 text-xs text-slate-500">{category}</div><p className="mt-1 text-sm text-slate-600">{description}</p></div><button type="button" data-testid={`run-report-mobile-${key}`} onClick={() => run(key)} className="rounded-lg border border-blue-200 px-3 py-1.5 text-sm font-bold text-blue-700">Run</button></div></article>)}</div></> : <div className="mt-4 rounded-lg bg-slate-50 px-4 py-6 text-center text-sm text-slate-500">No reports are available in this category.</div>}
        </section>
