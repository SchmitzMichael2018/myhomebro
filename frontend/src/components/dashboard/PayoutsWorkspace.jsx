...
function Metric({ label, amount, count, description, icon: Icon, tone }) {
  return <div className="rounded-xl border border-slate-200 bg-white p-4">
    ...
    <div className="mt-3 text-2xl font-black tabular-nums text-slate-950">{money(amount)}</div>
    {count != null ? <div className="mt-1 text-xs font-semibold text-slate-600">{count} payment{count === 1 ? "" : "s"}</div> : null}
    <div className="mt-3 border-t border-slate-100 pt-2 text-xs text-slate-500">{description}</div>
  </div>;
}
...
export default function PayoutsWorkspace({
  ...
  const metrics = [
    { key: "paid", supported: hasValue(summary, "total_paid_out") || hasValue(summary, "total_paid_amount"), label: "Paid Out", amount: summary.total_paid_out ?? summary.total_paid_amount, count: summary.payout_count ?? counts.paid, description: "Completed outgoing payments", icon: CheckCircle2, tone: "bg-emerald-50 text-emerald-700" },
    ...
    { key: "pending", supported: hasValue(summary, "total_pending_amount"), label: "Pending Approval", amount: summary.total_pending_amount, count: counts.pending, description: "Waiting for approval", icon: Clock3, tone: "bg-amber-50 text-amber-700" },
  ...
  return <div data-testid="dashboard-view-payouts" className="space-y-3">
    <section data-testid="dashboard-payouts-summary" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className={`mt-4 grid gap-3 sm:grid-cols-2 ${metrics.length >= 4 ? "xl:grid-cols-4" : "xl:grid-cols-3"}`}>{metrics.map((metric) => <Metric key={metric.key} {...metric} />)}</div>
    </section>

    <div className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_280px]">
      <div className="space-y-3">
        <section data-testid="dashboard-payouts-ledger" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {visibleRows.length ? <><div className="mt-4 hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-y border-slate-200 bg-slate-50 text-xs text-slate-500"><tr><th className="px-3 py-2">Recipient / Destination</th><th className="px-3 py-2">Payment Type</th><th className="px-3 py-2">Project / Source</th>{supportsRichAmounts ? <><th className="px-3 py-2">Gross</th><th className="px-3 py-2">Fees</th></> : null}<th className="px-3 py-2">Net Amount</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Date</th><th className="px-3 py-2">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{visibleRows.slice(0, 8).map((row) => <tr key={row.id} className="hover:bg-slate-50"><td className="px-3 py-2"><div className="font-bold text-slate-900">{row.recipient}</div></td><td className="px-3 py-2 text-slate-600">{row.paymentType}</td><td className="px-3 py-2"><div className="font-semibold text-slate-800">{row.source}</div><div className="text-xs text-slate-500">{row.source_label}</div></td>{supportsRichAmounts ? <><td className="px-3 py-2 tabular-nums">{row.gross != null ? money(row.gross) : "—"}</td><td className="px-3 py-2 tabular-nums text-slate-600">{row.fee != null ? money(row.fee) : "—"}</td></> : null}<td className="px-3 py-2 font-bold tabular-nums text-slate-900">{money(row.net)}</td><td className="px-3 py-2"><span className={`rounded-full px-2 py-1 text-xs font-bold capitalize ${statusTone(row.statusKey)}`}>{row.statusLabel}</span></td><td className="px-3 py-2 text-slate-600">{row.date ? new Date(row.date).toLocaleDateString() : "—"}</td><td className="px-3 py-2"><a href={historyHref(row)} className="font-bold text-blue-700">View</a></td></tr>)}</tbody></table></div><div className="mt-3 divide-y divide-slate-100 md:hidden">{visibleRows.slice(0, 8).map((row) => <article key={row.id} className="py-4 first:pt-0"><div className="flex items-start justify-between gap-3"><div><div className="font-bold text-slate-950">{row.recipient}</div><div className="mt-1 text-xs text-slate-500">{row.paymentType} · {row.source}</div></div><div className="text-right"><div className="font-black tabular-nums text-slate-950">{money(row.net)}</div><span className={`mt-1 inline-block rounded-full px-2 py-1 text-xs font-bold capitalize ${statusTone(row.statusKey)}`}>{row.statusLabel}</span></div></div><a href={historyHref(row)} className="mt-3 inline-flex min-h-9 items-center text-sm font-bold text-blue-700">View payment <ArrowRight aria-hidden="true" className="ml-1 h-4 w-4" /></a></article>)}</div></> : <div className="mt-4 rounded-lg bg-slate-50 p-4"><div className="font-bold text-slate-900">No outgoing payments in this period.</div><p className="mt-1 text-sm text-slate-500">Payouts, fees, refunds, and other supported disbursements will appear here as they are recorded.</p></div>}
        </section>

        <section data-testid="dashboard-payouts-upcoming" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-bold text-slate-950">Upcoming &amp; Pending</h2><p className="mt-1 text-sm text-slate-500">Outgoing payments that may need attention.</p>{attentionRows.length ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{attentionRows.map((row) => <a key={row.id} href={historyHref(row)} className="rounded-lg border border-slate-200 p-3 hover:border-blue-300"><div className="flex items-start justify-between gap-2"><div><div className="font-bold text-slate-900">{row.recipient}</div><div className="mt-1 text-xs text-slate-500">{row.paymentType}</div></div><span className={`rounded-full px-2 py-1 text-xs font-bold capitalize ${statusTone(row.statusKey)}`}>{row.statusLabel}</span></div><div className="mt-3 font-black tabular-nums text-slate-950">{money(row.net)}</div></a>)}</div> : <div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm text-slate-600">No outgoing payments need attention in this period.</div>}</section>
      ...
      <aside className="space-y-3">
        ...
        <section data-testid="dashboard-payouts-fees" className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><h2 className="text-lg font-bold text-slate-950">Fees This Period</h2>{platformFees != null ? <div className="mt-4 flex items-center justify-between"><div><div className="font-bold text-slate-800">Platform Fees</div><div className="mt-1 text-xs text-slate-500">Retained by MyHomeBro</div></div><div className="text-lg font-black tabular-nums text-slate-950">{money(platformFees)}</div></div> : <p className="mt-3 text-sm text-slate-500">No platform or processing fees were recorded in this period.</p>}</section>
