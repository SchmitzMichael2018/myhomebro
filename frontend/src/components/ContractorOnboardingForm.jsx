...
// v2025-11-09 — adds auto-redirect to /contractor/profile once Stripe is complete
import React, { useEffect, useState } from "react";
import toast from "react-hot-toast";
import api from "../api";

export default function ContractorOnboardingForm() {
  const [form, setForm] = useState({ business_name: "", name: "", email: "", phone: "", skills: "" });
  ...
  useEffect(() => {
    if (status?.charges_enabled && status?.details_submitted) {
      ...
      setTimeout(() => { window.location.href = "/contractor/profile"; }, 800);
    }
  }, [status]);
  ...
  const startOnboarding = async () => {
    ...
    try {
      ...
      if (!url) { setMsg("Onboarding URL not returned by server."); return; }
      window.location.href = url;
    } catch (err) {
      console.error(err);
      const detail =
        err?.response?.data?.detail || err?.response?.data?.error || err?.message || "Could not start onboarding.";
      setMsg(detail);
    } finally {
      setLoading(false);
    }
  ...
  const StatusBadge = ({ text, tone = "gray" }) => (
    <span className={`inline-flex items-center px-2 py-0.5 text-xs rounded
        ${tone === "green" ? "bg-green-100 text-green-800"
          ...
          : "bg-gray-100 text-gray-800"}`}>{text}</span>
  );
  ...
  const renderStatus = () => {
    ...
    if ("charges_enabled" in status) {
      pills.push(<StatusBadge key="charges" text={status.charges_enabled ? "Charges Enabled" : "Charges Disabled"} tone={status.charges_enabled ? "green" : "yellow"} />);
    }
    if ("details_submitted" in status) {
      pills.push(<StatusBadge key="details" text={status.details_submitted ? "Details Submitted" : "Details Pending"} tone={status.details_submitted ? "green" : "yellow"} />);
    }
    if ("requirements_due" in status && Array.isArray(status.requirements_due)) {
      pills.push(<StatusBadge key="reqs" text={status.requirements_due.length ? `${status.requirements_due.length} item(s) due` : "No outstanding requirements"} tone={status.requirements_due.length ? "yellow" : "green"} />);
    }
    return pills.length ? <div className="flex flex-wrap gap-2">{pills}</div> : <StatusBadge text="Status loaded" />;
  };

  return (
    <form onSubmit={(e) => { e.preventDefault(); startOnboarding(); }}
          className="max-w-lg mx-auto mt-16 p-8 bg-white rounded-2xl shadow-lg">
      <h2 className="text-2xl font-bold mb-2 text-center text-blue-800">Contractor Stripe Onboarding</h2>
      ...
      <div className="mb-4">
        ...
        <input name="business_name" value={form.business_name} onChange={handleChange}
               className="w-full border rounded px-3 py-2" placeholder="Your business name" />
      </div>
      <div className="mb-4">
      ...
      <div className="mb-4">
        ...
        <input name="phone" value={form.phone} onChange={handleChange}
               className="w-full border rounded px-3 py-2" placeholder="Phone number" />
      </div>
      <div className="mb-6">
        <label className="block font-semibold mb-1">Skills</label>
        <input name="skills" value={form.skills} onChange={handleChange}
               className="w-full border rounded px-3 py-2" placeholder="e.g., Flooring, Plumbing, Painting" />
      </div>
      ...
      <div className="text-xs text-gray-500 mt-4 text-center">
        You’ll be redirected to Stripe to complete verification. After returning, click <span className="font-semibold">Refresh Status</span>.
      </div>

      <div className="flex justify-center mt-3">
        <button type="button" onClick={fetchStatus} disabled={statusLoading}
