    ...
    >
      ...
      {activeBusinessView === "operations" ? (
        <div data-testid="dashboard-view-operations" className="space-y-3">
          <section data-testid="insights-operations-waiting-queue" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-950">Waiting Queue</h2><p className="mt-1 text-sm text-slate-500">Items that need attention.</p></div><a href="/app/business" className="inline-flex shrink-0 items-center gap-2 text-sm font-bold text-blue-700">View all queues <ArrowRight aria-hidden="true" className="h-4 w-4" /></a></div><div className="mt-4 overflow-x-auto pb-1"><div className="grid min-w-[880px] grid-cols-5 gap-3">
            {[
              ...
              ["reviews", "Milestone Review", overdueMilestoneCount, "/app/milestones", Star, "bg-blue-50 text-blue-700"],
            ...
            ].map(([key, label, count, href, Icon, tone]) => <a key={key} href={href} data-testid={`insights-operations-queue-${key}`} className="flex min-h-[76px] items-center gap-3 rounded-xl bg-slate-50 px-4 py-3 hover:bg-slate-100"><span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${tone}`}><Icon aria-hidden="true" className="h-5 w-5" /></span><span className="min-w-0 flex-1"><strong className="block text-xl font-black text-slate-950">{int(count)}</strong><span className="block text-sm font-bold text-slate-800">{label}</span></span><ChevronRight aria-hidden="true" className="h-4 w-4 shrink-0 text-slate-500" /></a>)}
          </div></div></section>
          ...
          <div className="grid items-start gap-3 xl:grid-cols-[1.55fr_0.75fr]">
            <section data-testid="insights-operations-active-work" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-950">Active Work</h2><p className="mt-1 text-sm text-slate-500">Projects in progress.</p></div><a href="/app/agreements" className="inline-flex shrink-0 items-center gap-2 text-sm font-bold text-blue-700">View all projects <ArrowRight aria-hidden="true" className="h-4 w-4" /></a></div>
              {activeWork.length ? <><div className="mt-4 hidden md:block"><table className="w-full text-left text-sm"><thead className="border-y border-slate-200 bg-slate-50 text-xs text-slate-500"><tr><th className="px-3 py-2">Project</th><th className="px-3 py-2">Stage</th><th className="px-3 py-2">Today&apos;s Task</th><th className="px-3 py-2">Due</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Open</th></tr></thead><tbody className="divide-y divide-slate-100">{activeWork.slice(0, 6).map((row, index) => <tr key={row.id || index}><td className="px-3 py-3 font-bold text-slate-900">{row.project_title || row.title || "Project"}</td><td className="px-3 py-3 text-slate-600">{row.stage || "—"}</td><td className="px-3 py-3 text-slate-700">{row.today_task || "—"}</td><td className="px-3 py-3 text-slate-600">{row.due_date || "—"}</td><td className="px-3 py-3 text-slate-600">{row.status || "Active"}</td><td className="px-3 py-3"><a href={row.open_href || "/app/agreements"} aria-label={`Open ${row.project_title || row.title || "project"}`}><ChevronRight aria-hidden="true" className="h-4 w-4 text-slate-600" /></a></td></tr>)}</tbody></table></div><div className="mt-4 divide-y divide-slate-100 md:hidden">{activeWork.slice(0, 6).map((row, index) => <a key={row.id || index} href={row.open_href || "/app/agreements"} className="block py-4 first:pt-0"><div className="flex items-center justify-between gap-3"><strong className="text-sm text-slate-900">{row.project_title || row.title || "Project"}</strong><ChevronRight aria-hidden="true" className="h-4 w-4 text-slate-500" /></div><div className="mt-2 grid grid-cols-2 gap-2 text-xs text-slate-500"><span>{row.stage || "Stage unavailable"}</span><span className="text-right">{row.due_date || "Due date unavailable"}</span><span className="col-span-2 text-slate-700">{row.today_task || "Today’s task unavailable"}</span></div></a>)}</div></> : <div data-testid="insights-operations-active-empty" className="mt-4 flex items-center gap-3 rounded-lg bg-slate-50 px-4 py-4"><Target aria-hidden="true" className="h-8 w-8 shrink-0 text-blue-500" /><div><div className="text-sm font-bold text-slate-900">No active projects today. You&apos;re caught up.</div><div className="mt-1 text-sm text-slate-500">Create a project or assign work to begin tracking execution.</div></div></div>}
            </section>

            <section data-testid="insights-operations-schedule-health" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-bold text-slate-950">Schedule Health</h2><p className="mt-1 text-sm text-slate-500">Current schedule signals.</p><div className="mt-4 divide-y divide-slate-100">
              {[
                ["On Schedule", "—", "Project detail unavailable", "bg-emerald-50 text-emerald-700"],
                ["Behind", int(overdueMilestoneCount), "Overdue milestones", "bg-amber-50 text-amber-700"],
                ["Ahead", "—", "Project detail unavailable", "bg-blue-50 text-blue-700"],
              ].map(([label, count, detail, tone]) => <div key={label} className="flex items-center gap-3 py-4 first:pt-0"><span className={`flex h-10 w-10 items-center justify-center rounded-full ${tone}`}><Clock3 aria-hidden="true" className="h-5 w-5" /></span><div className="min-w-0 flex-1"><div className="text-sm font-bold text-slate-900">{label}</div><div className="mt-0.5 text-xs text-slate-500">{detail}</div></div><div className="text-2xl font-black text-slate-950">{count}</div></div>)}
            </div><a href="/app/milestones" className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-blue-700">View schedule report <ArrowRight aria-hidden="true" className="h-4 w-4" /></a></section>
          </div>

          <section data-testid="insights-operations-daily-focus" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-bold text-slate-950">Daily Focus</h2><p className="mt-1 text-sm text-slate-500">Your top priority today.</p>{topNeedsAttention.length ? <div className="mt-4 flex flex-col gap-4 rounded-xl bg-emerald-50/70 p-5 sm:flex-row sm:items-center"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><Target aria-hidden="true" className="h-6 w-6" /></span><div className="min-w-0 flex-1"><h3 className="text-lg font-black text-emerald-800">{topNeedsAttention[0].title}</h3><p className="mt-1 text-sm text-slate-600">{topNeedsAttention[0].why}</p><div className="mt-3 flex flex-wrap gap-2"><span className={`rounded-md px-2.5 py-1 text-xs font-semibold ${topNeedsAttention[0].severity === "high" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>{topNeedsAttention[0].severity === "high" ? "Urgent" : "Needs attention"}</span>{topNeedsAttention[0].source_workspace ? <span className="rounded-md bg-white px-2.5 py-1 text-xs font-semibold text-slate-600">{topNeedsAttention[0].source_workspace}</span> : null}</div></div><a href={topNeedsAttention[0].open_url || "/app/agreements"} className="inline-flex h-11 shrink-0 items-center justify-center rounded-lg bg-blue-600 px-6 text-sm font-bold text-white hover:bg-blue-700">{topNeedsAttention[0].action_label || "Open item"}</a></div> : <div className="mt-4 flex items-center gap-3 rounded-xl bg-emerald-50 px-4 py-4"><Target aria-hidden="true" className="h-8 w-8 text-emerald-600" /><div><div className="font-bold text-emerald-900">You&apos;re caught up.</div><div className="mt-1 text-sm text-emerald-800">No operational item needs immediate attention.</div></div></div>}</section>
        </div>
      ) : null}
