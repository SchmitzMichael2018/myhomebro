...
function emitGuidedEvent(name, detail = {}) {
  ...
  window.dispatchEvent(
    new CustomEvent('mhb:analytics', { detail: { event: name, ...detail } })
  );
}
...
function AssistantLink({ role, step: activeStep, compact = false }) {
  ...
  const label = activeStep
    ? `Ask about ${activeStep.title.toLowerCase()}`
  ...
  return (
    <button
      ...
      onClick={() => {
        ...
        emitGuidedEvent('guided_assistant_opened', {
          role,
          step: activeStep?.id,
        });
        openAssistant({
          title: activeStep
            ? `Project Assistant · ${activeStep.workspace}`
            : 'Project Assistant · Guided Help',
          context,
        });
      }}
    >
...
export default function GuidedExperienceHub({ role }) {
  ...
  const derived = useMemo(
    () => deriveGuidedProgress(role, progress),
    [role, progress]
  );
  ...
  const start = () => {
    ...
    emitGuidedEvent(
      progress.status === 'paused' || progress.status === 'skipped'
        ? 'guided_journey_resumed'
        : 'guided_journey_started',
  ...
  const completeStep = () => {
    ...
    emitGuidedEvent('guided_step_completed', {
      role,
      step: derived.currentStep?.id,
    });
    save({
  ...
  const selectStep = (targetStep) => {
    const targetIndex = rolePath.steps.findIndex(
      (item) => item.id === targetStep.id
    );
    save({
      status: 'active',
      stepIndex: targetIndex >= 0 ? targetIndex : derived.stepIndex,
    });
  ...
  const openWorkspace = (targetStep) => {
    ...
    emitGuidedEvent('guided_workspace_opened', {
      role,
      step: targetStep.id,
      workspace: targetStep.workspace,
    });
  ...
  const confirmDialog = () => {
    if (dialog === 'skip') {
      ...
      emitGuidedEvent('guided_journey_skipped', {
        role,
        step: derived.currentStep?.id,
      });
    } else {
  ...
  return (
    <div className="guided-hub" data-testid="guided-experience-hub">
        ...
        <div className="guided-hub__progress-card">
          <div className="guided-hub__progress-copy">
            ...
            <strong aria-live="polite">
              {derived.completedCount} of {derived.total} walkthrough steps
            </strong>
          </div>
          <div
          ...
          <small>
            Walkthrough progress only. Your workspace records determine
      ...
      <GuidedVideoLibrary audience={role} title="Start with the essentials" />

      {recommended ? (
        <section
          className="guided-hub__recommended"
          aria-labelledby="guided-next-title"
        >
          <div className="guided-hub__recommended-icon">
          ...
          <div className="guided-hub__recommended-copy">
            ...
            <p className="guided-hub__missing">
              <strong>Focus:</strong> {recommended.missing}
            </p>
          </div>
          <div className="guided-hub__actions">
            <button
              className="guided-hub__primary-action"
              type="button"
              onClick={() => openWorkspace(recommended)}
            >
        ...
        <section
          className="guided-hub__complete"
        ...
        >
          ...
          <div>
            ...
            <p>
              You reviewed the core workflow. Optional guides remain available;
              workspace records and permissions are still the source of truth.
            </p>
            <p className="guided-hub__missing">
              <strong>Recommended first action:</strong> Review the workspace
              that changes most often for your role. Completion does not certify
              business or regulatory readiness.
            </p>
          </div>
          <button
            className="guided-hub__secondary-action"
            type="button"
            onClick={(event) => requestDialog('restart', event)}
          >
      ...
      >
        ...
        {!derived.isComplete && (
          <article
            className="guided-hub__focus-card"
            data-testid="guided-current-step"
          >
            <div className="guided-hub__step-number" aria-hidden="true">
              {derived.stepIndex + 1}
            </div>
            <div>
              ...
              <p className="guided-hub__missing">
                <strong>Useful outcome:</strong> {derived.currentStep.missing}
              </p>
            </div>
            <div className="guided-hub__actions">
              <button
                className="guided-hub__primary-action"
                type="button"
                onClick={() => openWorkspace(derived.currentStep)}
              >
              ...
              {derived.stepIndex > 0 ? (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    selectStep(rolePath.steps[derived.stepIndex - 1])
                  }
                >
                  Previous step
                </button>
        ...
        <ol
          className="guided-hub__stepper"
          aria-label={`${rolePath.label} guided stages`}
        >
          {rolePath.steps.map((item, index) => {
            const isCurrent =
              !derived.isComplete && index === derived.stepIndex;
            const isReviewed = derived.isComplete || index < derived.stepIndex;
            return (
              <li
                key={item.id}
                className={
                  isCurrent ? 'is-current' : isReviewed ? 'is-reviewed' : ''
                }
        ...
        <div className="guided-hub__journey-controls">
          {progress.status === 'active' ? (
            <button
              className="guided-hub__text-action"
              type="button"
              onClick={() =>
                save({ status: 'paused', stepIndex: derived.stepIndex })
              }
            >
          ...
          <button
            className="guided-hub__text-action"
            type="button"
            onClick={(event) => requestDialog('skip', event)}
          >
            Skip this guide
          </button>
          <button
      ...
      <section
        className="guided-hub__workspaces"
        aria-labelledby="guided-workspaces-title"
      >
        <div className="guided-hub__section-heading">
          <div>
                ...
                {group.steps.map((item) => (
                  <article key={item.id}>
                    ...
                    <div>
                      ...
                      <button type="button" onClick={() => selectStep(item)}>
                        Start walkthrough
                      </button>
      ...
      <section className="guided-hub__help" aria-labelledby="guided-help-title">
        ...
        <details>
          <summary>
            Does walkthrough progress change my real project data?
          </summary>
          <p>
            No. It only remembers where you are in this guide. Workspace
            records, permissions, approvals, and audit history remain
            authoritative.
          </p>
        </details>
        <details>
          ...
          <p>
            Yes. Your current role and walkthrough step are saved on this
            device, so returning to Guided Help resumes the same context.
          </p>
        </details>
        <details>
          <summary>What if an action is unavailable?</summary>
          <p>
            Your role and assigned permissions still control each workspace. Ask
            Project Assistant for context or use your role’s support page for
      ...
      <Modal
        ...
        onClose={closeDialog}
        title={
          dialog === 'skip' ? 'Skip this guide?' : 'Restart this walkthrough?'
        }
