...
function ReadinessRow({ item }) {
  ...
  return <article className="grid gap-2 border-t border-slate-100 px-2 py-2.5 first:border-t-0 md:grid-cols-[128px_minmax(210px,1fr)_85px_50px_125px] md:items-center md:py-1.5">
    ...
    <div className="flex items-start gap-2"><span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${item.iconTone}`}><Icon aria-hidden="true" className="h-3.5 w-3.5" /></span><div><h3 className="whitespace-nowrap text-sm font-bold text-slate-950">{item.title}</h3><p className="mt-0.5 text-xs leading-4 text-slate-500">{item.reason}</p></div></div>
...
export default function MarketingOverview({
  ...
  const primaryLabel = websitePublished ? (staleContentRisk ? 'Review Website' : 'Update Website') : blockerCount ? 'Continue Website Setup' : 'Review Before Publishing';
  ...
  return <section className="space-y-3" data-testid="marketing-overview-tab">
    <div className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_280px]">
      <div className="space-y-3">
        <section data-testid="marketing-readiness" className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-black text-slate-950">Marketing Readiness</h2><p className="mt-0.5 text-sm text-slate-500">Finish the highest-impact work first.</p></div><span className="whitespace-nowrap rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">{completedTasks.length} of {tasks.length} complete</span></div><div className="mt-2 hidden border-y border-slate-200 bg-slate-50 px-2 py-1.5 text-[11px] font-bold text-slate-500 md:grid md:grid-cols-[128px_minmax(210px,1fr)_85px_50px_125px]"><span>Priority</span><span>Task</span><span>Status</span><span>Impact</span><span>Action</span></div><div data-testid="marketing-readiness-list">{incompleteTasks.map((item) => <ReadinessRow key={item.key} item={item} />)}</div>{completedTasks.length ? <div data-testid="marketing-completed" className="mt-2 border-t border-slate-200 pt-2"><div className="text-xs font-bold text-slate-700">Completed</div><div className="mt-1 flex flex-wrap gap-2">{completedTasks.map((item) => <button key={item.key} type="button" onClick={item.onClick} className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs font-bold ${badgeTone.Completed}`}><CheckCircle2 className="h-3.5 w-3.5" />{item.title.replace('Download ', '')}</button>)}</div></div> : null}</section>

        <section data-testid="marketing-website-readiness" className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><h2 className="text-lg font-black text-slate-950">Website Readiness</h2><div className="mt-2 grid gap-2 sm:grid-cols-[1.5fr_repeat(3,90px)_auto] sm:items-center"><div><div className="text-sm font-bold capitalize text-slate-900">{websitePublished ? 'Published' : websiteStatus}</div><div className="text-3xl font-black text-slate-950">{readinessScore}% <span className="text-xs font-semibold text-slate-500">ready</span></div><div className="mt-1 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-blue-600" style={{ width: `${Math.min(readinessScore, 100)}%` }} /></div></div><div className="rounded-lg border border-red-100 p-2"><div className="text-lg font-black">{blockerCount}</div><div className="text-xs text-slate-500">Blockers</div></div><div className="rounded-lg border border-amber-100 p-2"><div className="text-lg font-black">{alertCount}</div><div className="text-xs text-slate-500">Alerts</div></div><div className="rounded-lg border border-emerald-100 p-2"><div className="text-lg font-black">{goodCount}</div><div className="text-xs text-slate-500">Good</div></div><button type="button" onClick={() => goToStep(websitePublished ? 'final' : 'website')} className="min-h-9 rounded-lg bg-blue-600 px-3 text-sm font-bold text-white">{primaryLabel}</button></div></section>
