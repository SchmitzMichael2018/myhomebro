...
export default function AgreementMilestoneStep({ step1Data, onBack, onSubmit }) {
  ...
  const validate = () => {
    for (let m of milestones) {
      ...
      if (!m.start_date || !m.completion_date)
        return "Both dates required for each milestone.";
      if (new Date(m.completion_date) < new Date(m.start_date))
        return "Completion date must be >= start date.";
      if (Number(m.days) + Number(m.hours) + Number(m.minutes) <= 0)
        return "Each milestone must have a time estimate.";
    }
    return null;
  };

  // Convert days/hours/minutes to seconds
  const durationToSeconds = (d, h, m) =>
    d * 86400 + h * 3600 + m * 60;
  ...
  const totalSeconds = milestones.reduce(
    (acc, m) =>
      acc + durationToSeconds(Number(m.days), Number(m.hours), Number(m.minutes)),
    0
  );
  ...
  const handleNext = () => {
    ...
    // Prepare milestones for parent (convert duration fields, trim title/desc)
    ...
    // Pass up to parent (step1Data is spread in parent or review step)
    onSubmit({
      milestones: outputMilestones,
      milestoneTotalCost: totalCost,
      milestoneTotalDuration: totalDurationStr,
    });
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto mt-6">
      <div className="mb-4 flex justify-between">
        ...
        <div>
          <span className="mr-6 font-semibold text-blue-800">
                ...
                <Draggable key={m.order} draggableId={String(m.order)} index={idx}>
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`border p-3 rounded space-y-2 mb-6 bg-white flex flex-col ${
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <span className="font-semibold">
                          <span className="cursor-grab mr-2 text-xl">⋮⋮</span>
                          Milestone #{m.order}
                        </span>
                        {milestones.length > 1 && (
                          <button
