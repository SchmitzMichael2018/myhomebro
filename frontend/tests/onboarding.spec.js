...
test('guided hub focuses one step, persists progress, and confirms destructive guide controls', async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole('heading', { name: 'Contractor owner path' })
  ).toBeVisible();
  await expect(page.getByTestId('guided-video-library')).toBeVisible();
  await expect(
    page.getByTestId('guided-video-card-create-contractor-profile')
  ).toBeVisible();
  await expect(
    page.getByTestId('guided-video-card-connect-stripe-and-get-paid')
  ).toBeVisible();
  await expect(page.getByTestId('guided-current-step')).toContainText(
    'Company Setup'
  );
  ...
  await expect(
    page.getByRole('link', { name: 'Back to Guided Help' })
  ).toBeVisible();
  ...
  await page
    .getByRole('button', { name: 'Mark walkthrough step reviewed' })
    .click();
  await expect(page.getByTestId('guided-current-step')).toContainText(
    'Marketing'
  );
  ...
  await expect(page.getByRole('dialog')).toContainText(
    'without marking any workspace work complete'
  );
...
test('contractor How-To Videos open in the guided media player', async ({
  page,
}) => {
  await installGuidedOnboardingMocks(page, 'contractor');
  await page.goto('/app/guided-onboarding', { waitUntil: 'domcontentloaded' });

  const profileVideo = page.getByTestId(
    'guided-video-card-create-contractor-profile'
  );
  await profileVideo.getByRole('button', { name: 'Watch video' }).click();

  const player = page.getByTestId('guided-video-player');
  await expect(player).toBeVisible();
  await expect(player).toHaveAttribute(
    'aria-label',
    /Create Your Contractor Profile/
  );
  await expect(page.getByTestId('guided-video-media')).toHaveAttribute(
    'src',
    /myhomebro-create-profile-howto/
  );
  await expect(
    page.getByRole('button', { name: /Turn captions (on|off)/ })
  ).toBeVisible();
});

test('guided hub renders role-specific journeys and workspace guides', async ({
  page,
}) => {
  ...
  for (const [role, heading, step] of [
    ...
    [
      ...
      'Review your property workspace',
    ],
    ['tenant', 'Tenant path', 'Start a maintenance request'],
    ['admin', 'Administrator path', 'Review administration'],
  ]) {
    currentRole = role;
    await page.goto('/app/guided-onboarding', {
      waitUntil: 'domcontentloaded',
    });
    await expect(page.getByRole('heading', { name: heading })).toBeVisible();
    await expect(page.getByTestId('guided-current-step')).toContainText(step);
    await expect(
      page.getByRole('heading', { name: 'Help where you work' })
    ).toBeVisible();
  }
});

for (const width of [320, 375, 390, 430]) {
  test(`guided hub remains usable at ${width}px and in standalone display mode`, async ({
    page,
  }) => {
    await page.addInitScript(() => {
