...
test('Marketing remains setup-only and hands public leads to Opportunities', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Opportunities', exact: true })).toHaveAttribute('href', '/app/opportunities');
...
test('opportunity review displays customer-requested estimate details as awaiting confirmation', async ({ page }) => {
  ...
  const proposedTimes = proposeDialog.getByLabel('Start time');
  await expect(proposedTimes).toHaveJSProperty('tagName', 'SELECT');
  expect(await proposedTimes.locator('option').evaluateAll((options) => options.every((option) => Number(option.value.slice(3)) % 15 === 0))).toBe(true);
