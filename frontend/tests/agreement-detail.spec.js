  ...
  return {
    ...
    planning_assumptions: {
      ...
      planning_capability_mix: [
        { capability: 'Carpentry', count: 2, available: 3 },
      ],
    },
    ...
    planning_validation_summary: {
      ...
      label: 'Needs Review',
      reason:
        'Timeline overlaps committed work or lacks complete planning context.',
      ...
      warnings: [
        {
          ...
          message: '1 signed/funded agreement overlaps this planned timeline.',
        },
      ],
      blockers: [],
    },
    ...
    readiness_checklist: [
      {
        ...
        ready: true,
        status: 'ready',
        detail: 'Signature requirements are satisfied.',
      },
      {
        label: 'Funding ready',
        ready: true,
        status: 'ready',
        detail: 'Escrow funding is complete.',
      },
      {
        label: 'Milestones available',
        ready: true,
        status: 'ready',
        detail: '1 milestone available.',
      },
      {
        label: 'Planning assumptions saved',
        ready: true,
        status: 'ready',
        detail: 'Agreement Wizard planning assumptions are available.',
      },
    ],
    blockers: [],
    warnings: [
      {
        ...
        message: 'Incidentals Reserve configured at $500.00.',
      },
    ],
    suggested_schedule: {
      start_date: '2026-08-03',
    ...
    material_readiness_notes: [
      {
        ...
        note: 'Confirm cabinet delivery.',
      },
    ],
    document_summary: {
      attachment_count: 2,
      customer_visible_attachment_count: 1,
    },
    planning_assumptions: agreementPayload().planning_assumptions,
    customer_visible_launch_summary_preview: {
      headline:
        'Kitchen Activation Project is being prepared for project kickoff.',
      start_date: '2026-08-03',
      finish_date: '2026-08-14',
      message:
...
async function installAgreementDetailMocks(
  page,
  {
    agreement = agreementPayload(),
    activation = activationPreviewPayload(),
  } = {}
) {
  ...
  await page.route(
    new RegExp(
      `/api/projects/agreements/${AGREEMENT_ID}/activation-preview/?(\\?.*)?$`
    ),
    async (route) => {
      await route.fulfill({
        status: 200,
  ...
  await page.route(
    new RegExp(
      `/api/projects/agreements/${AGREEMENT_ID}/funding_preview/?(\\?.*)?$`
    ),
    async (route) => {
      await route.fulfill({
        status: 200,
  ...
  await page.route(
    new RegExp(`/api/projects/agreements/${AGREEMENT_ID}/draws/?(\\?.*)?$`),
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
  ...
  await page.route(
    new RegExp(
      `/api/projects/agreements/${AGREEMENT_ID}/external-payments/?(\\?.*)?$`
    ),
    async (route) => {
      await route.fulfill({
        status: 200,
  ...
  await page.route(
    new RegExp(
      `/api/projects/agreements/${AGREEMENT_ID}/subcontractor-invitations/?(\\?.*)?$`
    ),
    async (route) => {
      await route.fulfill({
        status: 200,
  ...
  await page.route('**/api/projects/milestones/**', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify(agreement.milestones || []),
    });
  });
...
test('Agreement Workspace renders read-only activation preview', async ({
  page,
}) => {
  await installAgreementDetailMocks(page);

  await page.goto(`/app/agreements/${AGREEMENT_ID}/workspace?tab=activation`, {
    waitUntil: 'domcontentloaded',
  });

  await expect(
    page.getByTestId('agreement-workspace-panel-activation')
  ).toContainText('Activation Preview');
  await expect(
    page.getByTestId('agreement-workspace-panel-activation')
  ).toContainText('Preview only. No assignments or schedules are created.');
  await expect(
    page.getByTestId('agreement-planning-validation-banner')
  ).toContainText('Planning Needs Review');
  await expect(
    page.getByTestId('agreement-planning-validation-banner')
  ).toContainText('Review Timeline');
  await expect(
    page.getByTestId('activation-readiness-checklist')
  ).toContainText('Agreement signed');
  await expect(
    page.getByTestId('activation-readiness-checklist')
  ).toContainText('Funding ready');
  await expect(page.getByTestId('activation-crew-needs')).toContainText(
    'Carpentry'
  );
  await expect(page.getByTestId('activation-milestone-timeline')).toContainText(
    'Demo and prep'
  );
  await expect(
    page.getByTestId('activation-customer-launch-preview')
  ).toContainText('project kickoff');
});

test('Agreement Workspace summarizes milestone and invoice payment progress', async ({
  page,
}) => {
  const invoices = [
  ...
  await installAgreementDetailMocks(page, {
    agreement: agreementPayload({ invoices }),
  });

  await page.goto(`/app/agreements/${AGREEMENT_ID}/workspace`, {
    waitUntil: 'domcontentloaded',
  });
