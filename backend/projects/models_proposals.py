...
class ProposalLineItem(models.Model):
    ...
    notes = models.TextField(blank=True, default="")
    source_template = models.ForeignKey("projects.ProjectTemplate", on_delete=models.SET_NULL, null=True, blank=True, related_name="proposal_line_items")
    source_template_milestone = models.ForeignKey("projects.ProjectTemplateMilestone", on_delete=models.SET_NULL, null=True, blank=True, related_name="proposal_line_items")
    source_milestone_key = models.CharField(max_length=128, blank=True, default="")
    source_milestone_name = models.CharField(max_length=255, blank=True, default="")
    source_milestone_order = models.PositiveIntegerField(null=True, blank=True)
    source_allocation_percent = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
