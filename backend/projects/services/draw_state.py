...
def draw_workflow_label(workflow_status: str) -> str:
    mapping = {
        ...
        "paid": "Paid",
        "awaiting_release": "Payment Pending",
        "released": "Paid",
        "rejected": "Rejected",
...
def draw_workflow_message(draw) -> str:
    ...
    raw_status = _normalized_status(getattr(draw, "status", ""))
    if workflow_status == "payment_pending" and raw_status == DrawRequestStatus.AWAITING_RELEASE:
    ...
    if workflow_status == "paid" and raw_status == DrawRequestStatus.RELEASED:
...
def serialize_draw_workflow(draw) -> dict[str, Any]:
    ...
    return {
        ...
        "is_awaiting_release": _normalized_status(getattr(draw, "status", "")) == DrawRequestStatus.AWAITING_RELEASE,
    }
