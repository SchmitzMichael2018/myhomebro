...
from projects.services.amendment_application import prepare_accepted_change_amendment
...
class AmendmentRequestResponseView(APIView):
    ...
    def post(self, request, request_id: int):
        ...
        if (
            ...
            and amendment.initiated_by_role == "contractor"
            and is_homeowner
        ):
            try:
                workflow = prepare_accepted_change_amendment(amendment, actor=actor)
            except ValueError as exc:
                logger.warning("Accepted amendment request %s could not be prepared automatically: %s", amendment.id, exc)
                workflow = {"ok": False, "workflow_stage": "contractor_review_required", "detail": str(exc)}
        created_attachments = [
            AmendmentRequestAttachment.objects.create(
                amendment_request=amendment,
                agreement=agreement,
...
class ContractorAmendmentApplyView(APIView):
    ...
    def post(self, request, request_id: int):
        ...
        if not contractor or getattr(agreement, "contractor_id", None) != contractor.id:
            return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
        try:
            payload = prepare_accepted_change_amendment(amendment, actor=request.user)
        except ValueError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_409_CONFLICT)
        return Response(payload, status=status.HTTP_200_OK if payload.get("already_applied") else status.HTTP_201_CREATED)
