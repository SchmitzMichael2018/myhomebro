...
from projects.services.notification_center import ACTION_NEEDED_CATEGORIES, get_notification_queryset_for_user
...
def _filter_queryset(qs, filter_value: str):
    ...
    if normalized in {"action-needed", "action_needed"}:
        return qs.filter(Q(category__in=ACTION_NEEDED_CATEGORIES) | Q(event_type__in=ACTION_NEEDED_CATEGORIES))
    return qs
