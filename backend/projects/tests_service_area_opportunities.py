...
class ContractorServiceAreaOpportunitiesTests(TestCase):
    ...
    def test_relationship_and_market_readiness_are_location_trade_specific(self):
        ...
        self.assertEqual(by_area["Austin, TX"]["relationship"], "in_service_area")
        self.assertEqual(
            by_area["Dallas, TX"]["relationship"], "expansion_opportunity"
        )
        self.assertEqual(
            by_area["Albany, NY"]["relationship"], "outside_current_coverage"
        )
        self.assertTrue(
            by_area["Austin, TX"]["automatic_matching_approval"]
        )
        self.assertFalse(by_area["Austin, TX"]["automatic_matching_available"])
        self.assertFalse(
            by_area["Dallas, TX"]["automatic_matching_approval"]
        )
    ...
    def test_filters_sorting_and_pagination_are_server_side_and_stable(self):
        ...
        self.assertEqual(filtered["results"][0]["area"], "City 04, TX")
